...
export const questionsSchema = z.object({questions: z.array(z.object({id,question: text,reason:text,photoIds:z.array(id).max(20)}).strict()).max(5)}).strict();
...
export const storyRequestSchema = analysisSchema.extend({fictionGenre:z.enum(['literary','romance','mystery','fantasy','sf','healing']).optional(),chapterCount:z.number().int().min(1).max(10).nullable().optional(),creativity:z.union([z.literal(0),z.literal(25),z.literal(50),z.literal(75),z.literal(100)]).optional(),letterDetails:letterDetailsSchema.optional(),contextAnswers:z.array(z.object({questionId:id,question:text,answer:z.string().max(2000)}).strict()).max(5),storyType:z.enum(['diary','essay','travel','letter','fiction']),writingTone:z.enum(['plain','emotional','witty','cinematic','literary'])}).strict().superRefine((data,ctx)=>{
 if(data.chapterCount!=null&&(data.chapterCount<Math.ceil(data.photoAnalysis.length/4)||data.chapterCount>Math.floor(data.photoAnalysis.length/2)))ctx.addIssue({code:z.ZodIssueCode.custom,path:['chapterCount'],message:'한 챕터에 사진 2~4장이 들어가는 개수를 선택해 주세요.'});
});
