import type { Analysis, UploadedPhoto, MissingContextQuestion, GeneratedStory, StoryType, WritingTone, UserContextAnswer } from '@/types/story';
...
export const demoQuestions:MissingContextQuestion[]=[{id:'place',question:'이 시간은 어디에서, 어떤 계기로 남긴 기억인가요?',reason:'장소와 목적은 사진만으로 알 수 없습니다.'},{id:'people',question:'혼자 보낸 시간인가요, 누군가와 함께했나요?',reason:'동행인의 관계는 확인할 수 없습니다.'},{id:'moment',question:'사진 밖에서 가장 오래 기억하고 싶은 순간은 무엇인가요?',reason:'개인적인 기억을 듣습니다.'}];
...
export function makeDemoStory(type:StoryType,tone:WritingTone,answers:UserContextAnswer[],version=0):GeneratedStory {
 ...
 if(type==='letter'){paragraphs[0]='그날의 나에게. 네가 남긴 사진을 다시 펼쳐 보아. '+paragraphs[0];paragraphs[2]+=' 이 장면들을 너에게 다시 건네.';}
 if(type==='essay'&&version%2)paragraphs[2]+=' 사진이 남긴 것은 장면이고, 그 사이를 채우는 것은 지금 적는 문장이다.';
 return {title:titles[type],subtitle:version%2?'사진 사이에 남겨 둔 기록':'세 장의 사진, 하나의 이야기',coverPhotoId:'demo_02',sections:paragraphs.map((text,i)=>({heading:headings[type][i],paragraphs:memory[i]?[text,`사진 밖의 기억도 적어 둔다.\n${memory[i]}`]:[text],relatedPhotoIds:[demoPhotos[i].id]}))};
}
