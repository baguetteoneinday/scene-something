...
전개: 제공된 경험을 중심에 두고 구체적인 한 순간, 사용자가 말한 생각 또는 재구성 단계가 허용한 감정과 생각, 그 경험에 남은 의미를 연결한다. 뒤쪽 챕터를 개요나 요약으로 처리하지 않는다. 사진을 한 장씩 설명하는 문단을 만들지 않는다.
일관성: 선택한 분위기의 문장 리듬과 감정의 거리를 끝까지 유지한다. 문학적 표현을 점차 없애거나 마지막 챕터만 과장하지 않는다. 매 챕터에 도입과 교훈을 반복하지 않는다. 중간 챕터마다 여행 전체를 마무리하지 않는다.
사실성: 새로운 대화, 성격 이력, 행동은 만들지 않는다. 입력하지 않은 감정과 생각은 재구성 단계가 허용한 만큼만 더하며, 사용자가 직접 밝힌 마음을 바꾸지 않는다. 검토 때 잘못된 사실만 고치며 이미 좋은 문장의 리듬과 문단 구조를 보존한다. 모든 글을 똑같은 짧은 사실 나열로 다시 쓰지 않는다.`;
...
export const RECORD_STYLE_REFERENCE=`아래는 사용자가 선호한 문체의 비교용 예시다. 책, 도서관 등의 사건이나 감정을 현재 기록에 가져오지 말고 문장 호흡과 사고 전개만 참고한다.
“동네 도서관에서 읽다 만 책을 끝냈다. 그런데 마지막 문장이 잘 이해되지 않아서 두 번 읽었다. 끝까지 읽기는 했는데, 다 읽었다고 다 이해한 건 아니었다.
그래도 이번에는 모르는 문장을 그냥 넘기지 않았다. 책 한 권을 끝냈다는 것보다 그걸 더 기억하고 싶다.
꼭 바로 답을 찾아야 할 필요는 없겠다는 생각이 들었다. 이해가 안 되는 건 조금 더 생각해 보면 되니까. 책은 끝났지만 생각까지 끝낼 필요는 없었다.”
핵심 특징: 사실을 짧게 놓고 그 일이 나에게 어떤 의미였는지 자연스럽게 이어 간다. “그런데”, “그래도”, “~니까” 같은 일상적인 연결을 필요할 때 쓴다. 마지막 문장을 억지 명언으로 만들지 않는다. 사진 속 물건을 문단마다 나열하지 않는다. 선택한 분위기에 맞춰 표현을 조절하되 같은 사람이 쓴 목소리를 지킨다.`;
...
export async function writeRecordChapters(data:z.infer<typeof storyRequestSchema>,chapters:{photoIds:string[]}[],instructions:string,generate:typeof structured){
 ...
 const baseInstructions=instructions+'\nSHARED RECORD VOICE CONTRACT:\n'+RECORD_VOICE_CONTRACT+'\nSTYLE REFERENCE (style only, never source facts):\n'+RECORD_STYLE_REFERENCE;
